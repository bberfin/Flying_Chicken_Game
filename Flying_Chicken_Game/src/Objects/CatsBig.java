 ...
 * @author Berfin
 */
public class CatsBig extends GameObject 
{
    
    
    private BufferedImage cat_image;
    private BufferedImage bigCat;   
    private Handler handler;  //cuz we're going to need collision detection
    Random random = new Random();
    int choose = 0;


    public CatsBig(int x, int y, ObjectId id,Handler handler, SpriteSheet spriteSheet) {
        super(x, y, id,spriteSheet);
        this.handler=handler;

        BufferedImageLoader loader=new BufferedImageLoader();        
        bigCat=loader.loadImage("/bigCat.png");
    ...
    @Override
    public void render(Graphics g) // what our cats looklike
    {
        
        g.drawImage(bigCat, x, y,90,90,null);
